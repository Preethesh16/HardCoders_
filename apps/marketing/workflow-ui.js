...
(() => {
  ...
  function jobForm() {
    ...
    const testnetLimit = Number(model.runtime?.settlementLimitMajor);
    const limitAttribute = Number.isFinite(testnetLimit) && testnetLimit > 0 ? ` max="${escape(testnetLimit)}"` : "";
    const limitNote = limitAttribute ? ` Public TestNet demo limit: ${escape(testnetLimit.toFixed(2))} ${escape(fundingCurrency)} per deal.` : "";
    return `<section class="workspace-card action-card"><header><span>COMPANY INPUT · REQUIRED</span><b>NEW BRIEF</b></header><form class="workspace-form" data-workspace-form="job">
      ...
      <section class="single-delivery-editor" data-single-delivery><label><span>TOTAL PROJECT BUDGET · ${escape(fundingCurrency)}</span><input name="singleBudget" data-single-budget type="number" min="0.01"${limitAttribute} step="0.01" required placeholder="Enter the amount"></label><p>Complete the work once, submit one final evidence file, and release the full escrow after company approval.${limitNote}</p><button type="button" data-download-job-brief>DOWNLOAD JOB BRIEF PDF ↓</button></section>
  ...
  function applicationForm(job) {
    const testnetLimit = Number(model.runtime?.settlementLimitMajor);
    const priceLimit = Number.isFinite(testnetLimit) && testnetLimit > 0 ? ` max="${escape(testnetLimit)}"` : "";
    return `<section class="workspace-card action-card"><header><span>YOUR PROPOSAL</span><b>PRIVATE UNTIL SUBMITTED</b></header><form class="workspace-form" data-workspace-form="apply">
      ...
      <div class="field-grid"><label><span>PROPOSED PRICE · ${escape(job?.fundingCurrency ?? job?.budgetCurrency ?? "PAYER CURRENCY")}</span><input name="proposedPrice" type="number" min="0.01"${priceLimit} step="0.01" required placeholder="Enter the amount"></label><label><span>DELIVERY · DAYS</span><input name="deliveryDays" type="number" min="1" max="365" required placeholder="21"></label></div>
